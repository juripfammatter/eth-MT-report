...
The experiments are implemented in IsaacLab~\cite{d} with the ANYmal D quadrupedal agent and the RSL-RL framework~\cite{d} for training the policy with PPO~\cite{d}. The following four experiments use standard observation and action spaces which are detailed in Appendix~\ref{sec:appendix_obs_act}. The task space used for our method's random point sampling and goal definition is a 3D box defined in the $(x,y,z)$ space, i.e.\ $\phi(s)$ is the projection to $(x,y,z)$. We found this to be the most effective task space which is both convenient for goal definition while providing a trivial Euclidean distance metric for the nearest neighbor search.
...
\begin{figure}[h]
    ...
    \begin{subfigure}[c]{0.45\linewidth}
        ...
        \caption{Distribution over routes taken in successful episodes.}
    ...
    \begin{subfigure}[c]{0.25\linewidth}
        ...
        \caption{Trajectories of 512 evaluation episodes, colored by route.}
